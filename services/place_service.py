...
# ----------------------------------------------------------------------
def get_places():
    """기관 목록 [{id, slug, full_name, short_name, created_at}, ...] (최신순).
    비밀번호 해시는 응답에 포함하지 않음."""
    rows = get_super_db().execute(
        "SELECT id, slug, full_name, short_name, created_at"
...
def add_place(slug: str, full_name: str, short_name: str, password: str):
    ...
    try:
        db.execute(
            ...
            " VALUES (?, ?, ?, ?, ?)",
            (slug, full_name, short_name, generate_password_hash(password), now),
        )
        db.commit()
