...
root = tk.Tk()  # 전역 선언
root.withdraw()  # 처음에는 숨겨놓고, get_club_activity 내에서 재사용
is_fullscreen = True  # 전역 선언
...
def disable_fullscreen():
    root.overrideredirect(False)  # 테두리 복원
    root.geometry("1000x800")  # 일반 창 크기로 지정 (원하는 크기로 설정 가능)
    root.state("normal")  # 혹시 최소화되었을 경우 복원
...
def show_timed_message(title, message, timeout=30, level="info"):
    def close_message():
    ...
    #root.deiconify()  # 창 띄우기
    msg_window = tk.Toplevel(root)
    msg_window.title(title)
    msg_window.geometry("400x150")
    msg_window.resizable(False, False)
    msg_window.attributes("-topmost", True)  # 항상 위에

    msg_label = tk.Label(msg_window, text=message, font=(ft, 12), wraplength=380)
    msg_label.pack(pady=20)

    confirm_button = tk.Button(msg_window, text="확인", font=(ft, 12), command=msg_window.destroy)
    confirm_button.pack()

    # 타이머 설정: timeout 초 후 자동 닫힘
    msg_window.after(timeout * 1000, close_message)

def show_success(message):
    root.withdraw()
    show_timed_message("활동일지 프로그램", message, timeout=10, level="info")
    
def show_warning(message):
    root.withdraw()
    show_timed_message("활동일지 프로그램 오류", message, timeout=30, level="warning")
